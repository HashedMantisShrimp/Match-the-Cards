...
{
    internal static bool connectedToServer { get; private set; } = false;
    ...
    private static IMongoCollection<BsonDocument> lbCollection; //TODO: Create a Schema/Model script for collections
    ...
    internal static async Task SaveLeaderBoard(string playerListJSON)
    {
        ...
        const string docID = "lb00"; //TODO: Make it globally accessible
        var leaderBoardList = new BsonDocument()
        {
            {"_id", docID },
            {"list", data }
        };
        ...
        var update = Builders<BsonDocument>.Update.Set("list ", leaderBoardList);
        var options = new UpdateOptions { IsUpsert = true };

        UpdateResult result = await lbCollection.UpdateOneAsync(filter, update, options);

        Debug.Log($"<color=yellow> Operation {nameof(lbCollection.UpdateOneAsync)} returned result: ");
        Debug.Log(result);
